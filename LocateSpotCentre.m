...
function location = LocateSpotCentre(idx,s, CenterType, img_source)
format long;
...
function location = Gaussian2DFit(XYdata, Intensities)
    ...
    StartPoint = [(X(1)+X(end))/2, (Y(1)+Y(end))/2, 5, 5, 10, 600];% follow orders in Gaussian2DFunction, start points 
